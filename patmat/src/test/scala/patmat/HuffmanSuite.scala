...
class HuffmanSuite extends FunSuite {
  ...
  test("encode a very short text correctly") {
    new TestTrees {
      assert(encode(t1)("ab".toList) === List(0, 1))
    }
  }

  test("decode a very short bits list correctly") {
    new TestTrees {
  ...
  test("quick encode and encode of a very short text should be identity") {
    new TestTrees {
