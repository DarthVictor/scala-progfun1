...
class Empty extends TweetSet {
  ...
  def mostRetweetedAcc(mostRet: Tweet): Tweet = mostRet
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(mostRet: Tweet): Tweet = {
    val newMostRet = if(elem.retweets > mostRet.retweets) elem else mostRet
    right.mostRetweetedAcc(left.mostRetweetedAcc(newMostRet))
  }
}
